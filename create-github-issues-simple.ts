 ...
 * Creates GitHub issues for all Kiro tasks using GitHub CLI with proper escaping
 */
class SimpleGitHubIssuesCreator {
  ...
  private async createIssueForTask(task: any): Promise<void> {
    try {
      ...
      // Write body to temporary file to avoid command line issues
      const tempFile = `/tmp/issue-body-${task.id}.md`;
      writeFileSync(tempFile, body);

      // Create issue using GitHub CLI with file input
      const command = `gh issue create --repo ${this.repository} --title "${title}" --body-file "${tempFile}"`;
...
async function main() {
  try {
    ...
    const creator = new SimpleGitHubIssuesCreator();
...
export { SimpleGitHubIssuesCreator };