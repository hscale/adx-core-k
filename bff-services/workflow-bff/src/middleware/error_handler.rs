...
// Custom error type for the Workflow BFF service
...
impl IntoResponse for BffError {
    fn into_response(self) -> Response {
        let (status, error_code, message) = match &self {
            ...
            BffError::Workflow(_) => (StatusCode::UNPROCESSABLE_ENTITY, "WORKFLOW_ERROR", self.to_string()),
            ...
            BffError::Temporal(_) => (StatusCode::INTERNAL_SERVER_ERROR, "TEMPORAL_ERROR", "Temporal service error".to_string()),
            BffError::WebSocket(_) => (StatusCode::INTERNAL_SERVER_ERROR, "WEBSOCKET_ERROR", "WebSocket service error".to_string()),
            BffError::Internal(_) => (StatusCode::INTERNAL_SERVER_ERROR, "INTERNAL_ERROR", "Internal server error".to_string()),
        };

        error!("Workflow BFF Error: {} - {}", error_code, message);
...
impl BffError {
    ...
    pub fn api_client<E: Into<anyhow::Error>>(err: E) -> Self {
...
impl From<axum::Error> for BffError {
    fn from(err: axum::Error) -> Self {
        BffError::WebSocket(err.to_string())
    }
}

impl From<tokio::sync::broadcast::error::SendError<crate::services::websocket::WorkflowUpdate>> for BffError {
    fn from(err: tokio::sync::broadcast::error::SendError<crate::services::websocket::WorkflowUpdate>) -> Self {
        BffError::WebSocket(format!("Broadcast error: {}", err))
    }
}